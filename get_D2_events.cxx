...
int main(int argc, char **argv)
{
  ...
  /////////////Hadrons////////////////////
  TString NtupleName;
  TString VarList = "TargType:Q2:Nu:Xb:W:SectorEl:ThetaPQ:PhiPQ:Zh:Pt:W2p:Xf:T:P:T4:deltaZ:E:Ee:Pe:Ect:Sct:Ecr:Scr:evnt:Px:Py:Pz:Xe:Ye:Ze:Xec:Yec:Zec:TEc:ECX:ECY:ECZ:Pex:Pey:Pez:Ein:Eout:Eine:Eoute:pid:Betta:vxh:vyh:vzh:Px_ecpb:Py_ecpb:Pz_ecpb";
  ///////////////////////////////
  //////ELECTRONS/////
...
//////////////////////////////////////////
  ...
  for (Int_t k = 0; k < 10000; k++) 
  {
    ...
    if ((t->GetCategorization(0,tt.Data(),true)) == "electron") std::cout<<t->GetCategorization(0,tt.Data(),true)<<std::endl;

    
    if(nRows>0) std::cout<<t->Momentum(0)<<std::endl;
    ...
    if(nRows>0 && (t->GetCategorization(0,tt.Data(),true)) == "electron")// using modification flag true  
    {
      ...
      DataElec[14] = 0;//t->ElecVertTargTheta(tt);
      ...
      Float_t Mass =0;
      Bool_t D2_flag=kFALSE;
      for (Int_t i = 1; i < nRows; i++) 
      {
	Mass = TMath::Sqrt(t->Momentum(i)* t->Momentum(i)*(1.-t->Betta(i)*t->Betta(i))/t->Betta(i)/t->Betta(i));
	if (1.78<Mass&&Mass<1.98) D2_flag=kTRUE;
      }
      Int_t NmbPion = 0;
      for (Int_t i = 1; i < nRows; i++) 
      {
	...
	Bool_t D2_pid=kFALSE;
	Mass = TMath::Sqrt(t->Momentum(i)* t->Momentum(i)*(1.-t->Betta(i)*t->Betta(i))/t->Betta(i)/t->Betta(i));
	if (1.78<Mass&&Mass<1.98) D2_pid=kTRUE;
	std::cout<<Mass<<"####################"<<std::endl;
      	if (category == "gamma"&& D2_flag) 
        {
	  ...
	  vars[45] = t->Betta(i);
          ...
          vars[47] = t->Y(i);
          vars[48] = t->Z(i);
	  Float_t x=t->XEC(i),y=t->YEC(i),z=t->ZEC(i);
	  Float_t Rt =  TMath::Sqrt(x*x+y*y);
	  Float_t R =  TMath::Sqrt(x*x+y*y+(z-vert->Z())*(z-vert->Z()) );
	  Float_t phi = TMath::ATan2(y,x);
	  Float_t theta = TMath::ASin(Rt/R);
	  Float_t E_ecpb = TMath::Max(t->Etot(i),t->Ein(i)+t->Eout(i));
	  Float_t Px_ecpb = E_ecpb*Rt/R*TMath::Cos(phi);
	  Float_t Py_ecpb = E_ecpb*Rt/R*TMath::Sin(phi);
	  Float_t Pz_ecpb = E_ecpb*TMath::Cos(theta); 
	  vars[49] = Px_ecpb;
	  vars[50] = Py_ecpb;
	  vars[51] = Pz_ecpb;
    ...
    if(sim && t -> Id(0,1)==11 /*&& t -> Q2(1) > 1. && t -> W(1) > 2. && t -> Nu(1) / 5.015 < 0.85*/) 
    {
      ...
      DataElec[12] = k;
      DataElec[13] = t->ElecVertTarg();
      DataElec[14] =0;// t->ElecVertTargTheta(tt,1);
	  ...
	  vars[45] = t->Betta(i,1);
          ...
          vars[47] = t->Y(i,1);
          vars[48] = t->Z(i,1);
          vars[49] = 0;//t->ElecVertTargTheta(tt,1);
