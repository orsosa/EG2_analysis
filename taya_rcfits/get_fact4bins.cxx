...
Double_t *Delta_CFePb[3];
Double_t *Delta_CFePb_err[3];
Long_t Nent = 0;
...
int get_fact4bins()
{
  ...
  outfPt2<<"#Nu\tZh\tPt2\tfC\tfC_e\tfFe\tfFe_e\tfPb\tfPb_e\t##### fX = DeltaD2/DeltaX; DeltaX = (sig_rad/sig_born)_X\n";
  ...
  tD->Draw("Q2:Nu:Zh:Pt2:Fact_ex/tC.Fact_ex:Fact_ex/tFe.Fact_ex:Fact_ex/tPb.Fact_ex:Phi","","goffcandle");
  ...
  Delta_CFePb[0]=new Double_t[Nent];
  Delta_CFePb[1]=new Double_t[Nent];
  Delta_CFePb[2]=new Double_t[Nent];

  Delta_CFePb_err[0]=new Double_t[Nent];
  Delta_CFePb_err[1]=new Double_t[Nent];
  Delta_CFePb_err[2]=new Double_t[Nent];
  ...
  for (int k=0;k<Nent;k++)
  {
    ...
    for (int i=4;i<7;i++)
    {
      Delta_CFePb[i-4][k]=tD->GetVal(i)[k];
      //std::cout<<tD->GetVal(7)[k]<<endl;
      if ( (k%24==0)&&(k!=0) )
      {
	Double_t mean_Delta[3]={0,0,0};
	Double_t sigma_Delta[3]={0,0,0};
	for (int j=0;j<24;j++)
	{
	  mean_Delta[0]+=Delta_CFePb[0][k-j-1];
	  sigma_Delta[0]+=Delta_CFePb[0][k-j-1]*Delta_CFePb[0][k-j-1];
	  mean_Delta[1]+=Delta_CFePb[1][k-j-1];
	  sigma_Delta[1]+=Delta_CFePb[1][k-j-1]*Delta_CFePb[1][k-j-1];
	  mean_Delta[2]+=Delta_CFePb[2][k-j-1];
	  sigma_Delta[2]+=Delta_CFePb[2][k-j-1]*Delta_CFePb[2][k-j-1];
	...
	for (int j=0;j<24;j++)
	{
	 Delta_CFePb[0][k-j-1]= mean_Delta[0];
	 Delta_CFePb_err[0][k-j-1]=sigma_Delta[0];

	 Delta_CFePb[1][k-j-1]= mean_Delta[1];
	 Delta_CFePb_err[1][k-j-1]=sigma_Delta[1];

	 Delta_CFePb[2][k-j-1]= mean_Delta[2];
	 Delta_CFePb_err[2][k-j-1]=sigma_Delta[2];
  ...
  for (int knu=0;knu<3;knu++)//nu
  {
	...
	outfZ<<Delta_CFePb[0][ind]<<"\t"<<0.<<"\t";
	outfZ<<Delta_CFePb[1][ind]<<"\t"<<0.<<"\t";
	outfZ<<Delta_CFePb[2][ind]<<"\t"<<0.<<"\t";
	outfZ<<"\n";
      }
	...
	outfPt2<<Delta_CFePb[0][ind]<<"\t"<<0.<<"\t";
	outfPt2<<Delta_CFePb[1][ind]<<"\t"<<0.<<"\t";
	outfPt2<<Delta_CFePb[2][ind]<<"\t"<<0.<<"\t";
	outfPt2<<"\n";
      }
