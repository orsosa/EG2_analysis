...
void h10::Begin(TTree *tree)
{
  ...
  // TFidCut("Lorenzo_5GeV.txt");
   TFidCut("param50.dat"); //no distinction for vertex
   FileGam = new TFile("out.root","recreate");
...
Bool_t h10::Process(Long64_t entry)
{
   ...
   npi0 =0;   
   nphot=0;//initialize 0 photons for this event
	...
	&&trig_W>2
     ...
     // if (trig_vy_corr>-2.2 && trig_vy_corr<2 &&  trig_vz_corr>-25.54 && trig_vz_corr<-24.36)target=2; //Pb
      ...
      //  NpheSect(); not nessesary when applying Trig CC matching procedure
         if(kTRUE 
           &&trig_mom>0.75 // equivalent to ycut<0.85	        
           &&ec_ei[eci]>0.06
...
Bool_t h10::IsAPhoton(int track){
 ...
 if(q[track]==0){ 
   ...
   gam_E[nphot] = p[track];
   gam_Px[nphot] = p[track]*cx[track]; //EVNT 
   ...
   gam_Pxtrue[nphot] = gam_Etrue[nphot]*TMath::Cos(TMath::DegToRad()*gam_phi[nphot])*TMath::Sin(TMath::DegToRad()*gam_theta[nphot]); //ECPB
 ...
 if(kTRUE
    ...
    //&&gam_Eec[nphot]>0.15*0.273
     &&gam_Etrue[nphot]>0.1    //ATTENTION  Ecut > 0.1
