const int NT=3;
//set_format();
int get_MR_thesis_pictures_eta(TString fname="",TString xtitle="Var",TString outpf="out")
{
  ...
  for (int k=0;k<NT;k++)
  {
    ...
    auto hs_ac_px =  hs_ac->ProjectionX("hs_ac_px");
    hs_ac_px->SetMarkerStyle(kFullDotLarge);
    hs_ac_px->SetMarkerColor(col[k]);
    hs_ac_px->SetLineWidth(3);
    hs_ac_px->SetLineColor(col[k]);
    hs_ac_px->SetMarkerSize(3);
    auto hl_ac_px =  hl_ac->ProjectionX( "hl_ac_px");
    hl_ac_px->SetMarkerStyle(kFullDotLarge);
    hl_ac_px->SetMarkerColor(col[k]);
    hl_ac_px->SetLineWidth(3);
    hl_ac_px->SetLineColor(col[k]);
    hl_ac_px->SetMarkerSize(3);
    auto hac_s_px =  hac_s->ProjectionX("hac_s_px");
    hac_s_px->SetMarkerStyle(kFullDotLarge);
    hac_s_px->SetLineWidth(3);
    hac_s_px->SetMarkerSize(0);
    auto hac_l_px =  hac_l->ProjectionX("hac_l_px" );
    hac_l_px->SetMarkerStyle(kFullDotLarge);
    hac_l_px->SetLineWidth(3);
    hac_l_px->SetMarkerSize(0);
    auto hs_px =  hs->ProjectionX("hs_px");
    hs_px->SetMarkerStyle(kFullDotLarge);
    hs_px->SetMarkerColor(col[k]);
    hs_px->SetLineWidth(3);
    hs_px->SetLineColor(col[k]);
    hs_px->SetMarkerSize(3);
    auto hl_px =  hl->ProjectionX("hl_px");
    hl_px->SetMarkerStyle(kFullDotLarge);
    hl_px->SetMarkerColor(col[k]);
    hl_px->SetLineWidth(3);
    hl_px->SetLineColor(col[k]);
    hl_px->SetMarkerSize(3);
    ...
    hs_ac_px->Write("hN_AC_"+ST[k],TObject::kOverwrite);
    hl_ac_px->Write("hN_AC_D"+ST[k],TObject::kOverwrite);
    hac_s_px->Write("hAC_" + ST[k],TObject::kOverwrite);
    hac_l_px->Write("hAC_D" + ST[k],TObject::kOverwrite);
    hs_px->Write("hN_"+ST[k],TObject::kOverwrite);
    hl_px->Write("hN_D"+ST[k],TObject::kOverwrite);
